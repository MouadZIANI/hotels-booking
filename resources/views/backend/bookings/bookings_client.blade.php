                        ...
                        <tbody>
                            @foreach($bookings as $booking)
                                <tr style="min-width: 100px !important;">
                                    <td>{{ $booking->id }}</td>
                                    <td>
                                        <a href="{{ route('hotel-rooms', ['id' => $booking->roomType->hotel->id]) }}">
                                    ...
                                    <td>
                                        @if($booking->etat == -1)
                                            <a href="{{ route('booking.activate', ['id' => $booking->id]) }}" class="btn btn-success btn-sm btn-flat">
                                        ...
                                        @elseif($booking->etat == 0)
                                            <a href="{{ route('booking.cancel', ['id' => $booking->id]) }}" class="btn bg-orange-active btn-sm btn-flat">
                                                <i class="fa fa-minus-circle"></i>
                                            </a>
                                        @else
                                            <code><i>Valide</i></code>
                                        @endif()
                                        @if(!$booking->etat == 1)
                                            <form style="display: inline-block;" action="{{ route('bookings.destroy', ['id' => $booking->id]) }}" method="post">
                                                {{ csrf_field() }}
